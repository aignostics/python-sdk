...
def _generate_third_party_licenses(session: nox.Session, licenses_json_path: Path) -> None:
    ...
    """
    third_party_licenses = "# Attributions\n\n"
    third_party_licenses += "[//]: # (This file is generated by make docs from reports/licenses.json)\n\n"

    if not licenses_json_path.exists():
        third_party_licenses += "Please run `make audit` first to generate the necessary license information.\n"
        Path("ATTRIBUTIONS.md").write_text(third_party_licenses, encoding="utf-8")
    ...
    third_party_licenses += "This project includes code from the following third-party open source projects:\n\n"
    ...
    for pkg in licenses_data:
        ...
        third_party_licenses += f"## {name} ({version}) - {license_name}\n\n"

        if description:
            third_party_licenses += f"{description}\n\n"

        if url:
            third_party_licenses += f"* URL: {url}\n"

        if authors and authors != "UNKNOWN":
            third_party_licenses += f"* Author(s): {authors}\n"

        if maintainers and maintainers != "UNKNOWN":
            third_party_licenses += f"* Maintainer(s): {maintainers}\n"
        ...
        if license_text and license_text != "UNKNOWN":
            third_party_licenses += "### License Text\n\n"
            # Sanitize any backtick sequences by replacing triple backticks with alternative markdown fence
            sanitized_license_text = license_text.replace("```", "~~~")
            third_party_licenses += f"```\n{sanitized_license_text}\n```\n\n"

        # Add notice text if available
        notice_text = pkg.get("NoticeText", "")
        if notice_text and notice_text != "UNKNOWN":
            third_party_licenses += "### Notice\n\n"
            # Sanitize any backtick sequences in the notice text as well
            sanitized_notice_text = notice_text.replace("```", "~~~")
            third_party_licenses += f"```\n{sanitized_notice_text}\n```\n\n"

    third_party_licenses = third_party_licenses.rstrip() + "\n"
    Path("ATTRIBUTIONS.md").write_text(third_party_licenses, encoding="utf-8")

    session.log("Generated ATTRIBUTIONS.md file")


def _concatenate_readme(session: nox.Session) -> None:
    """Concatenate README partial files into a single README.md.
...
def docs(session: nox.Session) -> None:
    ...
    _generate_third_party_licenses(session, Path("reports/licenses.json"))
    _concatenate_readme(session)
