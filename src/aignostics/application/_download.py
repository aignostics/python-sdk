...
def download_item_artifact(  # noqa: PLR0913, PLR0917
    ...
    Fetches a fresh presigned download URL via
    ``GET /v1/runs/{run_id}/artifacts/{artifact_id}/file`` before downloading.

    Args:
        progress (DownloadProgress): Progress tracking object for GUI or CLI updates.
        run (Run): The application run that owns the artifact, used to fetch the
            presigned download URL from the platform API.
    ...
    artifact_download_url = run.get_artifact_download_url(artifact.output_artifact_id)
