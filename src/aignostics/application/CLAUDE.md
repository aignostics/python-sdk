...
human-readable stage labels. QuPath fields are only present when `ijson` is
installed (`has_qupath_extra = find_spec("ijson")`). See `_models.py` for fields.
...
`bucket` (storage), `utils` (DI/logging), and optional `qupath` (gated on `ijson`).
External deps and extras are declared in `pyproject.toml`; QuPath features require
the `qupath` extra (`ijson`).
